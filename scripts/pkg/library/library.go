...
func FetchDataFromGsheets(destFile string) {
	...
	if ok != true {
		fmt.Println("GSHEET_ID is not set.")
		os.Exit(1)
	}
	...
	for _, sheet := range sheets {
		...
		if err != nil {
			fmt.Printf("Could not download %s", sheet)
			continue
		}
		...
		for idx, record := range r {
			...
			if record[5] == "" || record[5] == "0" {
				fmt.Printf("Rating does not seem to be valid for %s\n", book.Title)
			...
			if record[6] == "" || record[6] == "0" {
				fmt.Printf("NumberOfPages does not seem to be valid for %s\n", book.Title)
			...
			if record[7] == "" || record[7] == "0" {
				fmt.Printf("YearPublished does not seem to be valid for %s\n", book.Title)
			...
			if record[8] == "" {
				fmt.Printf("DateRead does not seem to be valid for %s\n", book.Title)
			} else if dateRead, err := time.Parse("02/01/2006", strings.TrimSpace(record[8])); err != nil {
				fmt.Printf("error DateRead does not seem to be valid for %s\n", book.Title)
			} else {
				book.DateRead = &dateRead
			}

			if record[9] == "" {
				fmt.Printf("DateAdded does not seem to be valid for %s\n", book.Title)
